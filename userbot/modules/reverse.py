...
@errors_handler
async def okgoogle(img): #reverse search picture
    if not img.text[0].isalpha() and img.text[0] in ("."):
        ...
        if photo:
            ...
            yeet = [] #who the fuck named this yeet
            for i in images:
                k = requests.get(i)
                yeet.append(k.content)
            try:
                await img.client.send_file(entity=await img.client.get_input_entity(img.chat_id), file=yeet, reply_to=img)
            except TypeError:
                pass
