...
from userbot import BOTLOG, BOTLOG_CHATID, CMD_HELP, HOMIE_LIST, bot
from userbot.events import register, errors_handler

# =================== CONSTANT ===================
PP_TOO_SMOL = "`The image is too small`"
PP_ERROR = "`Failure while processing the image`"
NO_ADMIN = "`Tf? I am not an admin!`"
...
async def ban(bon):
    ...
    if user.id in HOMIE_LIST:
        await bon.edit("`Yo sar, I won't ban my homie!`")
        return
...
async def spider(spdr):
    ...
    if user.id in HOMIE_LIST:
        await spdr.edit("`Quite stupid idea trying to mute a homie...`")
        return
...
async def gspider(gspdr):
    ...
    if user.id in HOMIE_LIST:
        await gspdr.edit(
            "`Taping a homie globally breaks friendships...I rather don't do this.`")
        return
...
async def kick(usr):
    ...
    if user.id in HOMIE_LIST:
        await usr.edit("`Yo sar, I won't kick my homie!`")
        return
...
\n\n.pin\
\nUsage: Pins the replied/tagged message on the top the chat.\
\n\n.delusers\
