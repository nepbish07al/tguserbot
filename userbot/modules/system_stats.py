...
from userbot import bot, CMD_HELP, ALIVE_NAME, DB_SERVER, DB_URI
...
async def amireallyalive(alive):
    ...
    db = None
    db_valid = None
    db_server_status = system("pg_ctl status -D " + DB_SERVER)
    ...
    if db_valid and db_server_status  == 0:
    	db = "online"
    elif  DB_URI == "None":
    	db = "disabled"
    elif db_server_status >= 1:
    	db = "offline"
    elif db_valid is None:
    	db = "Connection to DB failed"
...
CMD_HELP.update(
    {"stats": ".stats\
    \nUsage: Counts statistics from owner."})
